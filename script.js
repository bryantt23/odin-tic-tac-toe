...
class Gameboard {
  ...
  constructor() {
    this.#gameBoardArray = this.#emptyBoard();
  }
  ...
  #checkWinner(playerMark) {
    const lines = [
      ...
      [0, 1, 2].map(index => this.#gameBoardArray[index][index]),
      [0, 1, 2].map(index => this.#gameBoardArray[index][2 - index])
    ];
    ...
    if (status !== GAME_STATUS.NOT_A_DRAW) {
      ...
      switch (status) {
        case GAME_STATUS.DRAW:
          gameOverMessage = `${this.#playerX.value} tied with ${
            this.#playerO.value
          }`;
          break;
        case GAME_STATUS.X_WINS:
          gameOverMessage = `${this.#playerX.value} defeated ${
            this.#playerO.value
          }`;
          break;
        case GAME_STATUS.O_WINS:
          gameOverMessage = `${this.#playerO.value} defeated ${
            this.#playerX.value
          }`;
          break;
      }
